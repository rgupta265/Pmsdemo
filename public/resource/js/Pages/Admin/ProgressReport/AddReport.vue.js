  ...
  methods: {
    getUserData: function getUserData() {
      ...
      axios.get("getUserData/" + this.token).then(function (response) {
        ...
        _this.checkReportStatus(_this.userData.user_id);
    ...
    checkReportStatus: function checkReportStatus(id) {
      var _this4 = this;

      axios.get("checkReport/" + id).then(function (response) {
        _this4.repCycle = response.data;
      });
    },
    isDisabled: function isDisabled(pms) {
      return !this.pmsAttrData.map(function (item) {
        return item.pms_attr_id;
      }).includes(pms);
    },
    filerCycle: function filerCycle(cycle) {
      return !this.repCycle.map(function (item) {
        return item.report_cycle;
  ...
  return _c("div", {
    ...
    return _c("option", {
      directives: [{
        name: "show",
        rawName: "v-show",
        value: _vm.filerCycle(op),
        expression: "filerCycle(op)"
      }],
      key: item,
      domProps: {
        value: op.value
    ...
    return _c("tr", {
      ...
      return _c("option", {
        directives: [{
          name: "show",
          rawName: "v-show",
          value: _vm.isDisabled(pms.id),
          expression: "isDisabled(pms.id)"
        }],
        key: ind,
        domProps: {
          value: pms.id
